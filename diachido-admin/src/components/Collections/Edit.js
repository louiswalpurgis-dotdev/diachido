...
import { CategoryIcon, EditIcon, UploadImage } from '../Icons';
...
export default function Edit({ collection }) {
    ...
    const validateInput = (name, description, index, imageUrl, iconImage) => {
    ...
    const uploadImages = async (imageUrl, iconImage, collection) => {
        let imageURL = collection.imageURL || '';
        let icon = collection.icon || '';
        if (imageUrl !== collection.imageURL) {
            imageURL = await uploadImage(imageUrl, 'collections');
        }
        if (iconImage !== collection.icon) {
            icon = await uploadImage(iconImage, 'collections');
        }
        return { imageURL, icon };
    };

    const updateCollectionDocument = async (name, description, index, imageURL, icon) => {
        await updateDocumentFields('collections', {
            name,
            description,
            index,
            imageURL,
    ...
    const deleteOldImages = async (collection) => {
        if (imageUrl !== collection.imageURL) {
            await deleteImage(collection.imageURL);
        }
        if (iconImage !== collection.icon) {
            await deleteImage(collection.icon);
        }
    };

    const handleChange = async function () {
        if (!validateInput(name, description, index, imageUrl, iconImage)) return;

        try {
            setIsLoading(true);
            const { imageURL, icon } = await uploadImages(imageUrl, iconImage, collection);
            await updateCollectionDocument(name, description, index, imageURL, icon);
            await deleteOldImages(collection);
